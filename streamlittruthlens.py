...
def comprehensive_video_analysis(video_path: str, source_url: str) -> Tuple[VideoAnalysisFeatures, List[AdvancedDetectionFeatures]]:
    ...
    try:
        ...
        if not frames:
            ...
            default_video = VideoAnalysisFeatures(
        ...
        # Analyze each frame
        frame_analyses = []
        for frame in frames[:5]:  # Limit to first 5 frames for performance
            frame_features = comprehensive_ai_detection(frame, source_url)
            frame_analyses.append(frame_features)
        
        # Video-specific analysis
        temporal_score = analyze_temporal_consistency(frames)
        motion_score = analyze_motion_vectors(frames)
        deepfake_score = analyze_deepfake_indicators(frames)
        
        video_features = VideoAnalysisFeatures(
            temporal_consistency_score=temporal_score,
            motion_vector_anomalies=motion_score,
            frame_transition_artifacts=0.5,  # Simplified
            optical_flow_irregularities=0.5,  # Simplified
            compression_pattern_analysis=0.5,  # Simplified
            facial_morphing_detection=deepfake_score,
            lip_sync_consistency=0.5,  # Simplified
            deepfake_indicators=deepfake_score,
            generation_timestamp_analysis=0.5,  # Simplified
            frame_interpolation_artifacts=motion_score
        )
...
def extract_video_frames(video_path: str) -> List[np.ndarray]:
    ...
    try:
        ...
        frames = []
        frame_count = 0
        max_frames = 10  # Limit for performance
...
def analyze_motion_vectors(frames: List[np.ndarray]) -> float:
    """Analyze motion vectors for anomalies"""
    try:
        if len(frames) < 2:
            return 0.5
        ...
        # Normalize and return anomaly score
        motion_anomaly = min(1.0, np.std(motion_scores) / (np.mean(motion_scores) + 1e-6))
        return motion_anomaly
        
    except Exception:
        return 0.5

def analyze_deepfake_indicators(frames: List[np.ndarray]) -> float:
    """Analyze deepfake indicators in video frames"""
    try:
        if not frames:
            return 0.5
        ...
        deepfake_scores = []
        for frame in frames:
            # Simplified deepfake detection based on face consistency
            gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
            
            # Edge analysis for face boundaries
            edges = cv2.Canny(gray, 50, 150)
            edge_density = np.sum(edges) / edges.size
            
            # Higher edge density around faces may indicate deepfakes
            deepfake_score = min(1.0, edge_density * 10)
            deepfake_scores.append(deepfake_score)
        
        return np.mean(deepfake_scores) if deepfake_scores else 0.5
...
def download_video_from_url(url: str) -> Optional[str]:
    ...
    try:
        ...
        # In production, you'd use yt-dlp for platform support
        
        # Direct download for simple video URLs
        if url.endswith(('.mp4', '.avi', '.mov', '.mkv', '.webm')):
            headers = {
        ...
        # For platforms like YouTube, TikTok, etc., you'd need yt-dlp
        # This is a placeholder that returns None for unsupported URLs
        st.warning("Video platform not supported in demo. Please upload a video file instead.")
        return None
...
if __name__ == "__main__":
    main()pixel_noise_variance=0.5, frequency_domain_anomalies=0.5, edge_sharpness_consistency=0.5,
            compression_artifacts=0.5, texture_analysis_score=0.5, color_histogram_anomalies=0.5,
            gradient_consistency=0.5, local_binary_patterns=0.5, neural_texture_patterns=0.5,
            upsampling_artifacts=0.5, attention_map_irregularities=0.5, latent_space_signatures=0.5,
            exif_consistency_score=0.5, timestamp_plausibility=0.5, color_profile_analysis=0.5,
